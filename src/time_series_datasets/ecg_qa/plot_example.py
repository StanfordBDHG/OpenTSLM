...
def draw_ecg(ecg, lead=1, ax=None):
    ...
    ax.vlines(np.arange(start_time, end_time, 0.2), ylims[0], ylims[1], colors="r", alpha=1.0, linewidth=0.8)
    ax.hlines(np.arange(ylims[0], ylims[1], 0.5), start_time, end_time, colors="r", alpha=1.0, linewidth=0.8)
    
    # Minor grid lines (every 0.04s and 0.1mV)
    ax.vlines(np.arange(start_time, end_time, 0.04), ylims[0], ylims[1], colors="r", alpha=0.3, linewidth=0.4)
    ax.hlines(np.arange(ylims[0], ylims[1], 0.1), start_time, end_time, colors="r", alpha=0.3, linewidth=0.4)
