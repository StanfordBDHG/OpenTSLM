...
class HARCoTQADataset(QADataset):
    ...
    def _get_text_time_series_prompt_list(self, row) -> list[TextTimeSeriesPrompt]:
        ...
        if torch.isnan(series).any() or torch.isinf(series).any():
            print("❌ Invalid data detected in HAR CoT sample")
        ...
        prompts = []
        for i, (time_series_label, time_series, mean, std) in enumerate(
            zip(TIME_SERIES_LABELS, series_norm.tolist(), means.squeeze().tolist(), stds.squeeze().tolist())
        ):
