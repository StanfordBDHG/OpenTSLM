...
class TSQADataset(QADataset):
    def _load_splits(self) -> tuple[Dataset, Dataset, Dataset]:
        ...
        train, val = train_val.train_test_split(test_size=VAL_FRAC / (1 - TEST_FRAC), seed=42).values()
    ...
    def _get_text_time_series_prompt_list(self, row) -> list[TextTimeSeriesPrompt]:
        ...
        return [
            TextTimeSeriesPrompt(
                f"This is the time series, it has mean {mean_val:.4f} and std {std_val:.4f}.", series.tolist()
            )
        ]
...
if __name__ == "__main__":
    ...
    train_values = [(el[0], el[1] / len(train)) for el in Counter(map(lambda x: x["post_prompt"], train)).items()]
    print("train", train_values)
    val_values = [(el[0], el[1] / len(val)) for el in Counter(map(lambda x: x["post_prompt"], val)).items()]
    print("val", val_values)
    test_values = [(el[0], el[1] / len(test)) for el in Counter(map(lambda x: x["post_prompt"], test)).items()]
