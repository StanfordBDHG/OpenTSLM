...
def does_ptbxl_exist():
    ...
    return (
        os.path.exists(PTBXL_DIR)
        and os.path.exists(PTBXL_RECORDS_DIR)
        and os.path.exists(os.path.join(PTBXL_DIR, "ptbxl_database.csv"))
    )
...
def clone_ecg_qa():
    ...
    subprocess.run(["git", "clone", ECG_QA_URL, ECG_QA_DIR], check=True)
...
def download_ptbxl():
    ...
    if not os.path.exists(ptbxl_zip_path):
        ...
        try:
            ...
            subprocess.run(["wget", "-O", ptbxl_zip_path, PTBXL_ZIP_URL], check=True)
    ...
    for req_file in required_files:
        ...
        if not os.path.exists(file_path):
            raise FileNotFoundError(
                f"Required PTB-XL file {req_file} not found in {PTBXL_DIR}. "
    ...
    if not os.path.exists(PTBXL_RECORDS_DIR):
        ...
        raise FileNotFoundError(
            f"PTB-XL records directory not found at {PTBXL_RECORDS_DIR}. "
            f"PTB-XL dataset may be corrupted or incomplete.\n"
...
def get_ptbxl_ecg_path(ecg_id: int) -> str:
    ...
    return os.path.join(PTBXL_RECORDS_DIR, f"{int(ecg_id / 1000) * 1000:05d}", f"{ecg_id:05d}_hr")
    ...
    def load_split_data(split_name: str) -> list[dict]:
        ...
        for json_file in tqdm(json_files, desc=f"Loading {split_name} JSON files"):
            ...
            try:
                ...
                raise ValueError(f"Invalid JSON in file {json_path}: {e!s}") from e
            except Exception as e:
                raise RuntimeError(f"Failed to read JSON file {json_path}: {e!s}") from e
            ...
            for sample_idx, sample in enumerate(tqdm(split_data, desc=f"Processing {json_file}", leave=False)):
                ...
                if not isinstance(ecg_ids, list):
                    raise ValueError(
                        f"Expected 'ecg_id' in sample {sample_idx} of {json_path} to be a list, got {type(ecg_ids)}"
                    )

                if not ecg_ids:
                    raise ValueError(
                        f"Sample {sample_idx} in {json_path} has empty ecg_id list. "
                ...
                for ecg_id in ecg_ids:
                    ...
                    if os.path.exists(ecg_path + ".dat"):
                        ...
                        raise FileNotFoundError(
                            f"ECG file not found: {ecg_path}.dat (ECG ID: {ecg_id}). "
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Loaded ECG-QA PTB-XL dataset:")
        ...
        if len(train) > 0:
            ...
            if sample.get("clinical_contexts"):
