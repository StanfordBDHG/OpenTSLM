...
"""
    ...
    def _load_ecg_data(cls, ecg_path: str) -> tuple[np.ndarray, str, str]:
        ...
        if ecg_path not in cls._ecg_data_cache:
            try:
                ...
                raise RuntimeError(f"Failed to read ECG record from {base_path}: {e!s}") from e
    ...
    def _process_ecg_lead(cls, ecg_path: str, lead_idx: int) -> tuple[np.ndarray, float, float]:
        ...
        if cache_key not in cls._processed_ecg_cache:
            # Load raw ECG data
            ecg_signal, _original_freq, _target_freq = cls._load_ecg_data(ecg_path)

            # Extract lead signal
            lead_signal = ecg_signal[:, lead_idx] if len(ecg_signal.shape) > 1 else ecg_signal
            ...
            if std_val > 1e-6:  # Avoid division by zero
                ...
                print(
                    f"Warning: Lead {lead_idx} in file {ecg_path} has very low std deviation ({std_val}), signal may be flat"
                )
            ...
            if np.any(np.isnan(normalized_signal)) or np.any(np.isinf(normalized_signal)):
                raise ValueError(
                    f"Invalid values (NaN/Inf) in normalized signal for lead {lead_idx} in file {ecg_path}"
                )
    ...
    def _get_text_time_series_prompt_list(self, row) -> list[TextTimeSeriesPrompt]:
        ...
        for i, ecg_path in enumerate(ecg_paths):
            # Load ECG data to determine number of leads
            ecg_signal, _original_freq, _target_freq = self._load_ecg_data(ecg_path)
            ...
            if len(ecg_signal.shape) == 1:
                ...
                if ecg_signal.shape[1] < 12:
                    print(
                        f"Warning: ECG file {ecg_path} has only {ecg_signal.shape[1]} leads, expected 12 for standard ECG"
                    )
            else:
            ...
            for lead_idx in range(n_leads):
                ...
                if len(normalized_signal) != 1000:
                    print(
                        f"Warning: Lead {lead_idx} in file {ecg_path} has {len(normalized_signal)} samples, expected 1000 for 100Hz"
                    )
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to create TextTimeSeriesPrompt for lead {lead_name} in file {ecg_path}: {e!s}"
                    ) from e
    ...
    def _format_sample(self, row):
        ...
        if "template_id" in row and row["template_id"] is not None:
            try:
                formatted_sample["possible_answers"] = ECGQACoTQADataset.get_possible_answers_for_template(
                    row["template_id"]
                )
            except Exception:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(dataset) > 0:
            ...
            print(
                "Sample answer (rationale):",
                sample["answer"][:200] + "..." if len(sample["answer"]) > 200 else sample["answer"],
            )
            ...
            if "rationale" in sample:
                print(
                    "CoT Rationale:",
                    sample["rationale"][:100] + "..." if len(sample["rationale"]) > 100 else sample["rationale"],
                )
