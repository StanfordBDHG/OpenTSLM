...
def download_and_extract_har_cot():
    ...
    if os.path.exists(HAR_COT_TRAIN_CSV) and os.path.exists(HAR_COT_VAL_CSV) and os.path.exists(HAR_COT_TEST_CSV):
    ...
    if not os.path.exists(HAR_COT_ZIP):
        ...
        try:
            with urllib.request.urlopen(HAR_COT_RELEASE_URL) as response:
                ...
                with (
                    open(HAR_COT_ZIP, "wb") as f,
                    tqdm(total=total, unit="B", unit_scale=True, desc="Downloading har_cot.zip") as pbar,
                ):
            ...
            raise RuntimeError(f"Failed to download HAR CoT dataset: {e}") from e
    ...
    try:
        ...
        raise RuntimeError(f"Failed to extract HAR CoT dataset: {e}") from e

    # Verify all CSV files exist
    if not all([os.path.exists(HAR_COT_TRAIN_CSV), os.path.exists(HAR_COT_VAL_CSV), os.path.exists(HAR_COT_TEST_CSV)]):
...
def ensure_har_cot_dataset():
    ...
    """
    if not (os.path.exists(HAR_COT_TRAIN_CSV) and os.path.exists(HAR_COT_VAL_CSV) and os.path.exists(HAR_COT_TEST_CSV)):
