...
if __name__ == "__main__":
    ...
    train_values = [(el[0], el[1] / len(train)) for el in Counter(x["post_prompt"] for x in train).items()]
    print("train", train_values)
    val_values = [(el[0], el[1] / len(val)) for el in Counter(x["post_prompt"] for x in val).items()]
    print("val", val_values)
    test_values = [(el[0], el[1] / len(test)) for el in Counter(x["post_prompt"] for x in test).items()]
