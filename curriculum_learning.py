...
class CurriculumTrainer:
    ...
    def _initialize_model(self):
        ...
        if self.model_type == "EmbedHealthSP":
            model = EmbedHealthSP(llm_id=self.llm_id, device=self.device).to(
                self.device
            )
    ...
    def _train_stage(
        ...
        if eval_only:
            ...
            for epoch in range(start_epoch, num_epochs + 1):
                ...
                # Training
                self.model.train()
                if output_memory_use:
                    # Currently allocated memory (by tensors)
                    allocated = torch.cuda.memory_allocated() / 1024**2  # in MB

                    # Memory reserved by the caching allocator (may be larger than allocated)
    ...
    def stage1_mcq(
        ...
        return self._train_stage(
            ...
            batch_size=batch_size,
            eval_only=eval_only,
            output_memory_use=output_memory_use,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Curriculum Learning for EmbedHealth Models"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Enable memory use logging",
    )
