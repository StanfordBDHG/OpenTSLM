...
class TestHARCoTLoader(unittest.TestCase):
    ...
    def test_example_data(self):
        ...
        for axis in ["x_axis", "y_axis", "z_axis"]:
            self.logger.info(
                f"{axis}: length={len(sample[axis])}, first 5: {sample[axis][:5]}, last 5: {sample[axis][-5:]}"
            )
        self.logger.info("=" * 80)
...
class TestHARCoTQADataset(unittest.TestCase):
    ...
    def test_time_series_text_includes_mean_std(self):
        ...
        for i, text in enumerate(sample["time_series_text"]):
            ...
            if not mean_match:
                self.logger.error(f"DEBUG: {text!r}")
            self.assertIsNotNone(mean_match, f"No mean value found in: {text!r}")
            std_match = re.search(r"std\s+(-?\d+\.\d+)", text)
            if not std_match:
                self.logger.error(f"DEBUG: {text!r}")
            self.assertIsNotNone(std_match, f"No std value found in: {text!r}")
    ...
    def test_example_data_QA(self):
        ...
        self.logger.info(f"Label: '{sample['label']}'")
        pre_prompt_preview = (
            sample["pre_prompt"][:200] + "..." if len(sample["pre_prompt"]) > 200 else sample["pre_prompt"]
        )
