...
class TestECGQACotLoader(unittest.TestCase):
    ...
    def test_example_data(self):
        ...
        if "rationale" in sample:
            rationale_preview = (
                sample["rationale"][:200] + "..." if len(sample["rationale"]) > 200 else sample["rationale"]
            )
...
class TestECGQACoTQADataset(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.train_no_comparison = self.ECGQACoTQADataset(
            split="train", EOS_TOKEN="", max_samples=10, exclude_comparison=True
        )
    ...
    def test_exclude_comparison_functionality(self):
        ...
        for sample in self.train_no_comparison:
            ...
            self.assertFalse(
                question_type.startswith("comparison"),
        ...
        self.logger.success(
            f"Exclude comparison test passed. Regular dataset has comparison questions: {has_comparison}"
        )
            ...
            for i, sample in enumerate(dataset):
                ...
                for j, ts in enumerate(time_series):
                    ...
                    if actual_length != expected_length:
                        self.logger.warning(
                            f"Dataset {dataset_name}, sample {i}, time series {j}: "
                            f"Expected {expected_length} samples, got {actual_length}"
                        )

                    # All values should be numeric
                    for val in ts:
                        self.assertIsInstance(val, (int, float), f"Non-numeric value in time series: {val}")
    ...
    def test_error_handling_missing_fields(self):
        ...
        try:
            ECGQACoTQADataset(split="train", EOS_TOKEN="", max_samples=1)
