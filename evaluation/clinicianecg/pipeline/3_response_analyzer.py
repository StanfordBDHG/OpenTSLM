...
class ECGResponseAnalyzer:
    ...
    def _extract_sample_responses(
        self, worksheet, reviewer_name: str, reviewer_initials: str, sheet_name: str
    ) -> dict | None:
        ...
        try:
            ...
            sample_data.update({"template_id": template_id, "ecg_id": ecg_id, "sample_id": sample_id})
            ...
            sample_data["completion_status"] = (
                "Completed" if completed_questions == 3 else "In Progress" if completed_questions > 0 else "Not Started"
            )
            ...
            if len(completed_df) > 0:
                ...
                for i in range(1, 4):
                    ...
                    if question_col in completed_df.columns:
                        ...
                        reviewer_stats[f"question_{i}_positive_rate"] = (
                            positive_count / len(responses) * 100 if len(responses) > 0 else 0
                        )

                stats["by_reviewer"][reviewer] = {"total_assessments": len(completed_df), **reviewer_stats}
        ...
        if multi_reviewed_samples:
            ...
            if agreement_stats["agreement_by_question"]:
                ...
                total_comparisons = sum(
                    q["total_comparisons"] for q in agreement_stats["agreement_by_question"].values()
                )
                if total_comparisons > 0:
    ...
    def _is_complete_disagreement(self, response1: str, response2: str) -> bool:
        ...
        negative_responses = ["None identified", "Completely incorrect logic", "No did not use any relevant background"]
    ...
    def _plot_agreement_analysis(self, df: pd.DataFrame, output_path: Path):
        ...
        bars = ax.bar(range(len(questions)), agreement_rates, color=["#3498db", "#e74c3c", "#2ecc71"])
            ...
            if self.summary_stats:
                ...
                if "model_performance" in stats:
                    ...
                    for question, data in stats["model_performance"].items():
                        ...
                        f.write(
                            f"  - Positive Evaluations: {data['positive_evaluations']} ({data['positive_rate']:.1f}%)\n"
                        )
                        f.write(
                            f"  - Partial Evaluations: {data['partial_evaluations']} ({data['partial_rate']:.1f}%)\n"
                        )
                        f.write(
                            f"  - Negative Evaluations: {data['negative_evaluations']} ({data['negative_rate']:.1f}%)\n"
                        )
    ...
    def _plot_individual_subfigures(self, df: pd.DataFrame, output_path: Path):
        ...
        _fig_a, ax = plt.subplots(1, 1, figsize=(7, 4.5))  # Increased width from 6 to 7
        ...
        bars3 = ax.bar(
            ...
            negative_rates,
            width,
            bottom=np.array(positive_rates) + np.array(partial_rates),
            label="Negative",
            color="#e74c3c",
        )
        ...
        for i, (bar1, bar2, bar3) in enumerate(zip(bars1, bars2, bars3)):
            if positive_rates[i] > 1:  # Show label for any segment larger than 1%
                ax.text(
                    ...
                    bar1.get_height() / 2,
                    f"{positive_rates[i]:.0f}%",
                    ha="center",
                    va="center",
                    fontweight="bold",
                    color="white" if positive_rates[i] > 15 else "black",
                    fontsize=12,
                )
            if partial_rates[i] > 1:
                ax.text(
                    ...
                    f"{partial_rates[i]:.0f}%",
                    ha="center",
                    va="center",
                    fontweight="bold",
                    color="white" if partial_rates[i] > 15 else "black",
                    fontsize=12,
                )
            if negative_rates[i] > 0.5:  # Lower threshold for negative rates to catch small values like 1.2%
                ax.text(
                    ...
                    f"{negative_rates[i]:.0f}%",
                    ha="center",
                    va="center",
                    fontweight="bold",
                    color="white" if negative_rates[i] > 15 else "black",
                    fontsize=12,
                )
        ...
        _fig_b, ax = plt.subplots(1, 1, figsize=(7, 4.5))  # Increased width from 6 to 7
        ...
        if assessment_matrix and len(reviewers) > 0:
            ...
            cbar.set_label(
                "Positive Assessment Rate", rotation=270, labelpad=20, fontsize=16
            )  # Increased from 14 to 16
            cbar.ax.tick_params(labelsize=14)  # Increased from 12 to 14
        else:
            ax.text(
                0.5, 0.5, "No reviewer data available", ha="center", va="center", transform=ax.transAxes, fontsize=16
            )
        ...
        _fig_c, ax = plt.subplots(1, 1, figsize=(9, 7))  # Increased width from 8 to 9 and height from 6 to 7
        ...
        if "agreement_analysis" in self.summary_stats:
            ...
            if disagreement_data:
                ...
                if question_disagreements:
                    ...
                    for p in patterns:
                        if " - " in p and ("↔" in p or " vs " in p):
                            ...
                            label, bar_color = self._create_colored_disagreement_label(
                                question_part, response1, response2
                            )
                    ...
                    ax.legend(
                        handles=legend_elements, loc="lower right", bbox_to_anchor=(1.0, 0.02), fontsize=14
                    )  # Smaller font and custom position
                    ...
                    ax.text(
                        0.5,
                        0.5,
                        "No disagreement patterns found",
                        ha="center",
                        va="center",
                ...
                ax.text(
                    0.5,
                    0.5,
                    "No disagreement analysis available",
                    ha="center",
                    va="center",
                    transform=ax.transAxes,
                    fontsize=12,
...
def main():
    ...
    print("\nAnalysis complete!")
    print("Results saved in: ../analysis_results/")
