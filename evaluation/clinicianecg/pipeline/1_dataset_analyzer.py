...
class ECGDatasetAnalyzer:
    ...
    def collect_all_samples(self):
        ...
        template_dirs = sorted(
            [d for d in self.dataset_path.iterdir() if d.is_dir() and d.name.startswith("template_")]
        )

        for template_dir in template_dirs:
            template_num = int(template_dir.name.split("_")[1])

            sample_dirs = sorted([d for d in template_dir.iterdir() if d.is_dir() and d.name.startswith("sample")])
    ...
    def generate_assignment_summary(
        self, assignments: dict[str, list[dict]], sample_assignments: dict[int, list[str]]
    ) -> pd.DataFrame:
    ...
    def print_assignment_statistics(self, assignments: dict[str, list[dict]]):
        ...
        self.config["reviews_per_sample"]
        ...
        for template_id in sorted(all_templates):
            counts = [template_distribution[reviewer].get(template_id, 0) for reviewer in self.reviewers]
...
def main():
    ...
    with open(assignments_path, "wb") as f:
        pickle.dump({"assignments": assignments, "sample_assignments": sample_assignments, "samples_data": samples}, f)
