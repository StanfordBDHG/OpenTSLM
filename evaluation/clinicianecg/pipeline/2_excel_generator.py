...
class ECGReviewGenerator:
    ...
    def crop_ecg_whitespace(self, image_path: Path, temp_dir: Path, sample_id: str | None = None):
    ...
    def create_sample_sheet(self, wb: Workbook, sample_data: dict, sheet_name: str):
        ...
        try:
            if sample_data["ecg_plot_path"].exists():
                ...
                temp_image_path, cropped_size = self.crop_ecg_whitespace(
                    sample_data["ecg_plot_path"], temp_dir, unique_id
                )
                ...
                if temp_image_path and temp_image_path.exists():
                    ...
                    target_display_height = min(
                        950, max(900, target_reviewer_height * 1.1)
                    )  # 900-950px for clinical viewing
                    ...
                    ecg_image_row + rows_needed

                    print(
                        f"ECG Image: {crop_width}x{crop_height} (cropped, aspect: {crop_aspect_ratio:.3f}) -> {img.width}x{img.height} display (900-950px)"
                    )
                    ...
                    ecg_image_row + rows_needed
                    print(
                        f"Maximum ECG (fallback): {orig_width}x{orig_height} -> {target_width}x{target_height} (maximum size)"
                    )
            else:
            ...
            ws[f"D{ecg_image_row}"] = "Error loading ECG image"
            ecg_image_row + 70
    ...
    def create_summary_sheet(self, wb: Workbook, reviewer_name: str, samples: list):
        ...
        for instruction in instructions:
            ...
            elif instruction.startswith(("Total", "Each")):
        ...
        metrics = [
            ...
            (
                "Completion %:",
                f'=IF({len(samples)}>0,ROUND(COUNTIF(E{start_data_row}:E{end_data_row},"Completed")/{len(samples)}*100,1),0)&"%"',
            ),
        ]
...
def main():
    ...
    print("Features:")
    print("  • ECG images sized at 900-950px height for clinical analysis")
    print("  • Enhanced cropping: top 8%, bottom 7%, left 5%, right 35% removed")
    print("  • Automatic status tracking: updates as questions are answered")
    print("  • Progress summary with completion metrics")
    print("  • Optimized column spacing for better ECG-to-reviewer flow")
    print("  • 3-column layout: Clinical → ECG → Assessment")
    print("  • Automatic initials propagation")
    print("  • Professional styling and validation")
    print(f"{'=' * 60}")
